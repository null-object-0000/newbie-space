  ...
  <div class="tool-page" :class="{ 'dark-mode': isDark, 'light-mode': !isDark }">
    ...
    <main class="tool-main">
      ...
      <section class="tool-header">
        ...
        <p>调整图片的宽度和高度，支持锁定宽高比和百分比缩放。图片只在浏览器本地处理，不会上传。</p>
      </section>
      ...
      <section v-if="originalImage" class="controls-card">
        ...
        <div class="control-group">
          <label>输出格式</label>
          <div class="mode-toggle" role="group" aria-label="输出格式">
            <button
      ...
      <section v-if="originalImage" class="action-card">
        ...
        <div class="action-buttons">
          ...
          <button class="primary-button" type="button" @click="applyResize">
            <Maximize2 :size="18" />
            <span>预览调整效果</span>
          </button>
          <button class="primary-button" type="button" :disabled="!resizedDataUrl" @click="downloadImage">
            <Download :size="18" />
            <span>下载 {{ outputFormat.toUpperCase() }}</span>
          </button>
          <button
            class="pipeline-button"
            type="button"
            :disabled="!resizedDataUrl"
            @click="sendToTransparent"
          >
...
async function sendToTransparent() {
  ...
  if (ok) {
    ...
    setTimeout(() => {
      window.location.href = '/tools/background-transparent'
    }, 300)
  } else {
...
async function tryLoadPipelineImage() {
  ...
  try {
    const blob = dataUrlToBlob(piped.dataUrl)
    const file = new File([blob], piped.fileName, { type: 'image/png' })
    await loadFile(file)
...
function downloadImage() {
  ...
  const downloadName = `${baseName}-${targetWidth.value}x${targetHeight.value}.${ext}`
